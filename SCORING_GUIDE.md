...
**How the Rapid Cloud Assessment tools calculate risk and posture scores.**

This repository contains **two independent scoring systems** — one for the live dashboard, one for the CSA report. They measure different things and are not directly comparable.
...
1. [The two scoring systems at a glance](#-the-two-scoring-systems-at-a-glance)
2. [Part 1 — Dashboard Risk Score](#-part-1--dashboard-risk-score-serverjs)
   - [Score bands (Maturity Model)](#-score-bands-maturity-model)
   - [Formula](#-formula)
   - [Weights & rationale](#%EF%B8%8F-weights--rationale)
3. [Part 2 — CSA Report Posture Score](#-part-2--csa-report-posture-score-lw_report_genpy)
   - [Global posture score](#-global-posture-score)
   - [Per-CSP gauge scores (AWS / Azure / GCP)](#-per-csp-gauge-scores-aws--azure--gcp)
   - [Score labels & colors](#-score-labels--colors)
   - [`_sc` — risky secrets explained](#-_sc--risky-secrets-explained)
4. [Worked examples](#-worked-examples)

---

## 🧭 The two scoring systems at a glance

| | Dashboard Risk Score | CSA Report Posture Score |
|---|---|---|
| **Where** | `rca_ui/server.js` | `lw_report_gen.py` |
| **Range** | `0 – 100` (risk) | `0 – 10` (posture) |
| **Direction** | ⬆️ higher = worse | ⬆️ higher = better |
| **Purpose** | Real-time operational view | Executive / customer deliverable |
| **Inputs** | Top identity risk, top CVE, critical alerts | Compliance, CVEs, MFA gaps, alerts, secrets |
| **Output** | Single number + posture % + stage | Global score + per-CSP gauges + labels |
