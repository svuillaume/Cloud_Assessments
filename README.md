...
By default the tool reports against the **CIS** benchmark. Use `--compliance-framework` to switch to a different framework:

```bash
python lw_report_gen.py --api-key-file mykey.json --compliance-framework NIST_800_53
```
...
| `CSA_CCM` | CSA Cloud Control Matrix v4.0.5 |

> **Note:** Framework availability depends on your FortiCNAPP subscription. If a framework is not available for your account the compliance section will be empty and a warning will be logged.
